...
beforeEach(() => {
  // Keep the suite hermetic. The market resolves versions through the
  // `undici` package, which the `globalThis.fetch` mocks below cannot
  // intercept once a proxy dispatcher is involved, and this machine exports
  // `http_proxy`. Blanking every proxy source (blank is "unset" for the
  // market) selects the direct path that honours the mocked fetch, and a
  // closed loopback registry keeps any accidental egress from reaching npm.
  for (const name of [
