...
Linux 支持兼容、扩展与增强三种模式。其托盘模式命令已启用，扩展 / 增强窗口通过 Electron 的 Window Controls Overlay 保留原生最小化 / 最大化 / 关闭按钮；上游内容会为命令栏让出顶部空间，避免与这些按钮重叠。

## 兼容模式

`dsh-desktop.mode` 默认为 `compatibility`。在 macOS 与 Windows 上，该模式会在当前 DSH profile 的官方 Web surface 上方创建一条独立的 36 CSS 像素 Desktop frame，并保留原生红绿灯或窗口按钮。居中的标识、模式 pill、拖动区域与图标操作只属于该 frame；完整官方页面从它下方开始，不参与 frame 的布局或安全区计算。Linux 的兼容模式保留普通原生 frame 作为 fallback。
...
扩展窗口会禁用官方上游 `ui-layout` root，并安装 Desktop 自有 root layout。该 layout 负责 sidebar、conversation、details、overlay 和 resize 几何，同时继续渲染官方 sidebar、conversation 与 details slot occupant。Desktop root 上方是一条固定的 36 CSS 像素操作栏；操作栏与 Desktop 自有 sidebar surface 组成连续的倒 L 形区域。conversation surface 位于倒 L 内侧，使用 10 像素内圆角，并让分隔线沿圆角绘制。
...
- Linux 支持全部三种呈现模式；扩展与增强模式依赖 Window Controls Overlay，并保留原生最小化 / 最大化 / 关闭按钮。
- 托盘终端会在 macOS、Windows 与 Linux 上提供私有 `dsh`、`pnpm` 与 `node` shim。除此之外，Host runtime 会在当前 Electron 进程的 `PATH` 中公开内置 `pnpm` 命令作为 ambient compatibility，并提供受管 `desktopPnpm` service；这些命令都不会加入系统 `PATH`。
