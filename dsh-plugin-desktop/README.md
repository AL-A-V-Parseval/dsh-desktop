...
Linux supports compatibility, extended, and enhanced modes. Its tray mode command is enabled, and the extended/enhanced windows use Electron's Window Controls Overlay so the native minimize/maximize/close buttons remain available; Linux window material can be `off` or the macOS/WWDC25 Liquid Glass look (solid window/content, frosted controls, overlays, and pop-up menus).
...
- Linux supports all three presentation modes; extended and enhanced rely on the Window Controls Overlay and can use `off` or the Liquid Glass material (solid surfaces, frosted controls, overlays, and pop-up menus).
