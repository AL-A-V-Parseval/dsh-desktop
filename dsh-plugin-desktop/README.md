...
Linux supports compatibility, extended, and enhanced modes. Its tray mode command is enabled, and the extended/enhanced windows use Electron's Window Controls Overlay so the native minimize/maximize/close buttons remain available; Linux window material can be `off` or `transparent`.
...
- Linux supports all three presentation modes; extended and enhanced rely on the Window Controls Overlay and can use `off` or `transparent` material.
