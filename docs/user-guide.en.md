...
- **Enhanced mode** retains its dedicated root registration and compact internal captions: macOS uses a 20-pixel content inset with a 32-pixel drag region, while Windows and Linux use a 32-pixel caption row. It does not reuse the independent extended frame.

macOS custom-window modes can turn the transparent material on or off. Windows can turn material off; Mica appears only when supported on Windows 11 build 22621 or newer. A legacy Windows Acrylic preference is safely treated as off and migrated when its settings file is writable. Changing mode or material restarts the application; it does not hot-swap root slots or native materials in a live renderer. Linux provides all three modes; extended and enhanced keep the native minimize/maximize/close buttons through the Window Controls Overlay, with material selectable between off and the macOS/WWDC25 Liquid Glass look (the window and content stay solid; controls, settings, dialogs, and pop-up menus are translucent frosted glass).
