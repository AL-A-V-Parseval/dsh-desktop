...
`)
  ...
  it('starts in the mode the not-yet-imported settings document is about to apply', () => {
    ...
    // Linux presents the custom shells in this fork, so its section merges like
    // every other platform's instead of being left to the import.
    const linuxExtended = temporaryHome()
    writeDesktopShellPreferences(linuxExtended, ['mode: compatibility'])
    writeFileSync(join(linuxExtended, 'settings.yaml'), ['desktop-shell:', '  mode: extended', ''].join('\n'))
    expect(prepareDesktopProfile(undefined, linuxExtended, 'linux').mode).toBe('extended')
